...
static int find_location(float result[], Color_t image[], uint8_t image_thrs[],
                         ...
                         int min_size, int max_size, int compute_orientation,
                         float orientation_offset, int downsample, int start_x,
                         int start_y, int end_x, int end_y, uint8_t mask[]) {
  ...
  for (y_ind = start_y; y_ind < end_y; y_ind += downsample) {
    for (x_ind = start_x; x_ind < end_x; x_ind += downsample) {
      ...
      float f =
          c->r * color_coefs[R] + c->g * color_coefs[G] + c->b * color_coefs[B];
...
static int process_image(float result[], Color_t image[], uint8_t image_thrs[],
                         int sx, int sy, float color_coefs[], uint8_t threshold,
                         int min_size, int max_size, int compute_orientation,
                         float orientation_offset, int downsample,
                         uint8_t mask[], int window_size) {
  ...
  if (downsample > 1) {
    ...
    find_location(result, image, image_thrs, sx, sy, color_coefs, threshold,
                  min_size_dwn, max_size_dwn, /*compute_orientation=*/0,
  ...
  find_location(result, image, image_thrs, sx, sy, color_coefs, threshold,
                min_size, max_size, compute_orientation, orientation_offset,
...
static PyObject *PY_find_location(PyObject *dummy, PyObject *args,
  ...
  static char *keywords[] = {"image",     "image_thrs",          "color_coefs",
                             "threshold", "compute_orientation", "size_lim",
                             "mask",      "orientation_offset",  NULL};

  if (!PyArg_ParseTupleAndKeywords(
          args, kw, "OO(fff)b|p(II)Of", keywords, &image, &image_thrs,
          &coefs[0], &coefs[1], &coefs[2], &threshold, &compute_orientation,
          &min_size, &max_size, &mask, &orientation_offset))
    goto fail;
  Py_INCREF(image);
  ...
  find_location(result, img_data, img_thrs_data, dims[0], dims[1], coefs,
                threshold, min_size, max_size, compute_orientation,
                orientation_offset, /*downsample=*/1,
                /*start_x=*/0, /*start_y=*/0, /*end_x=*/dims[0],
                /*end_y=*/dims[1], /*mask=*/NULL);
  Py_DECREF(image);
...
static PyObject *PY_process_image(PyObject *dummy, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kw, "OO(fff)b|p(II)OIIf", keywords,
                                   &image, &image_thrs, &coefs[0], &coefs[1],
                                   &coefs[2], &threshold, &compute_orientation,
                                   &min_size, &max_size, &mask, &downsample,
                                   &window_size, &orientation_offset))
    goto fail;
  ...
  process_image(result, img_data, img_thrs_data, dims[0], dims[1], coefs,
                threshold, min_size, max_size, compute_orientation,
                orientation_offset, downsample, /*mask=*/NULL, window_size);
